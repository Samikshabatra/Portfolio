/**
 * lucide dropped brand marks in v1, so GitHub and LinkedIn live here as inline
 * paths. Same size/prop shape as the lucide icons used everywhere else, so they
 * drop into the same layouts without adjustment.
 */

/** Accepts the same props lucide icons do, so they are interchangeable. */
type IconProps = Omit<React.SVGProps<SVGSVGElement>, "size"> & {
  size?: number;
  /** Accepted and ignored — these marks are solid fills, not strokes. */
  strokeWidth?: number;
};

export function GithubIcon({ size = 15, strokeWidth, ...rest }: IconProps) {
  void strokeWidth;
  return (
    <svg
      {...rest}
...
export function LinkedinIcon({ size = 15, strokeWidth, ...rest }: IconProps) {
  void strokeWidth;
  return (
    <svg
      {...rest}
